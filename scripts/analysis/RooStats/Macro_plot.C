...
// double pot_standard = 2.7148568e+20/(102757./107046.); run3 
// double pot_standard = 2.8326206e+20/(112042./111810.); run 1

/*
      SP / larcv 
Run1: 112042./111810.
Dirt-Run1: 59622./58652.
Run3: 102757/107046
*/
...
void MacroPlot(std::string mass_point){
    ...
    // We should have just one POT number without the necesity to re-scale every time...
    ...
    std::map<std::string,double> fermion_scalings ={
        {"0.01",12.7}, {"0.02",14.02},{"0.03",16.9},{"0.04",18.58},{"0.05",20.57},
        {"0.06",20.39}, {"0.07",18.76},{"0.08",15.71},{"0.09",13.3},{"0.1",12.3}

    };

    // Variable to plot and cuts
    std::string cut_vars = "n_vertex == 1 && n_tracks == 0 && n_showers == 1 && signal_score >= 0.9 ";
    std::string plot_var = "E_total";  
    std::string parent_directory = "/home/lmlepin/Desktop/dm_sets/dark_tridents_analysis/";
    std::string out_image_name = parent_directory + "Official/energy_distribution_run1_" + mass_point + ".png";
    std::string output_root_files= parent_directory + "Official/dt_hists_mass_" + mass_point+".root";
    bool in_progress_option=false;
    bool is_scalar = false;
    bool log_option=true;
    bool save_hist_option=true; 
    bool save_plot=true;
    char y_label[128] = "Number of events / 2 #times 10^{20} POT";
    char x_label[128] = "Energy [MeV]";
    // Get this from function, for signal provide this in config file (?)
    double pot_standard = 2.8326206e+20/(112042./111810.);
    double pot_dirt = 1.73939e+20/(59622./58652.);
    double pot_ext = 4.35574e+23;
    double pot_dt; 
    ...
    double expected_pot = 2.e20; // Run 1 POT  
    ...
    // Replace here for signal, standard, EXT and dirt 
    // Also we should have for run1, run2 and run3 
    auto fileName_neutrino = "/home/lmlepin/Desktop/dm_sets/background_studies/standard_NuMI_run1_fhc_CNN_scores_8441_steps.csv";
    auto fileName_b = "/home/lmlepin/Desktop/dm_sets/background_studies/dirt_NuMI_run1_CNN_scores_8441_steps.csv";
    //auto fileName_c = "EXT_NuMI.csv";
    ...
    std::cout << "Total simulated neutrinos: " << total_standard << std::endl;
    std::cout << "Total simulated dirt: " << total_dirt << std::endl;
    //std::cout << "Total simulated gamma: " << total_ext << std::endl;

    std::cout << "===============================" << std::endl;

    auto cut_dt = *filteredEvents_dt.Count()*(expected_pot/pot_dt);
    auto cut_standard = *filteredEvents_standard.Count()*(expected_pot/pot_standard);
    auto cut_dirt = *filteredEvents_dirt.Count()*(expected_pot/pot_dirt);
    //auto cut_ext = *filteredEvents_ext.Count()*(expected_pot/pot_ext);

    std::cout << "Total a/c DT: " << cut_dt << std::endl;
    std::cout << "Total a/c neutrinos: " << cut_standard << std::endl;
    std::cout << "Total a/c dirt: " << cut_dirt << std::endl;
    //std::cout << "Total a/c ext: " << cut_ext << std::endl;
    ...
    // Create histograms
    TH1D* h1 = new TH1D("standard","standard", nbins, min_val, max_val);
    TH1D* h2 = new TH1D("dirt","dirt", nbins, min_val, max_val);
    //TH1D* h3 = new TH1D("ext","ext", nbins, min_val, max_val);
    TH1D* h4 = new TH1D("signal","signal", nbins, min_val, max_val);
    ...
    std::cout<<"Number of neutrino events: " << h1->Integral() << std::endl;
    std::cout<<"Number of dirt events: " << h2->Integral() << std::endl;
    //std::cout<<"Number of EXT events: " << h3->Integral() << std::endl;
    ...
    std::string n_dt_string = "Dark tridents: "+ dt_string;
    std::string n_standard_string =  "Neutrinos: "+ standard_string; 
    std::string n_dirt_string = "Dirt: "+ dirt_string;
    //std::string n_ext_string = "Cosmics: "+ ext_string;
